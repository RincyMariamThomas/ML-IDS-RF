...
def train_model(X_train, y_train, param_grid):
    ...
    return grid_search, X_test_scaled, y_test, best_params

def evaluate_model(grid_search, X_test_scaled, y_test):
    """
    Evaluate the model's performance and print out the results.
    """
    # Make predictions
    y_pred_test_best = grid_search.best_estimator_.predict(X_test_scaled)
    ...
    print("Test Accuracy (without PCA) with Best Hyperparameters:", test_accuracy_best)
...
def main(dataset_path):
    ...
    # Train model
    grid_search, X_test_scaled, y_test, best_params = train_model(features, labels, param_grid)
